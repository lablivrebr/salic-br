...
class GerarRelatorioReuniaoController extends MinC_Controller_Action_Abstract {
    ...
    public function init() {
        ...
        if ($auth->hasIdentity()) { // caso o usu�rio esteja autenticado
            ...
            // pega as unidades autorizadas, org&atilde;os e grupos do usu�rio (pega todos os grupos)
            $grupos = $Usuario->buscarUnidadesAutorizadas($auth->getIdentity()->usu_codigo, 21);

            // manda os dados para a vis&atilde;o
            $this->view->usuario = $auth->getIdentity(); // manda os dados do usu�rio para a vis&atilde;o
            $this->view->arrayGrupos = $grupos; // manda todos os grupos do usu�rio para a vis&atilde;o
            $this->view->grupoAtivo = $GrupoAtivo->codGrupo; // manda o grupo ativo do usu�rio para a vis&atilde;o
            $this->view->orgaoAtivo = $GrupoAtivo->codOrgao; // manda o org&atilde;o ativo do usu�rio para a vis&atilde;o
        } // fecha if
        else {
